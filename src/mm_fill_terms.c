...
void
fluid_stress_fenep( double Pi[DIM][DIM],
		   STRESS_DEPENDENCE_STRUCT *d_Pi)
{  /*
  ...
  dbl s[MAX_MODES][DIM][DIM];           // Polymer stress tensor
  ...
  if ( pd->v[POLYMER_STRESS11] )
  {
    memset( s, 0, sizeof(dbl)*DIM*DIM*MAX_MODES);
    for (mode=0; mode<vn->modes; mode++) {
      trace[mode] = 0.;
      for (a = 0; a < VIM; a++) {
        for (b = 0; b < VIM; b++) {
          s[mode][a][b] = fv->S[mode][a][b];
        }
        trace[mode] += s[mode][a][a];
      }
    }
  }
      ...
      for(p=0; p<VIM; p++)
        {
		  ...
		  if(pd->v[POLYMER_STRESS11])
		      ...
		      for(mode=0; mode<vn->modes; mode++)
			{
			  ...
			  if(fenep)
			    {
			      d_Pi->T[p][q][j] += d_mup->T[j]/lambda*(f_fenep*s[mode][p][q]-a_fenep*(double)delta(p,q));
			    }			  
			}		      
		    }
		}
      ...
      for(p=0; p<VIM; p++)
        {
		  ...
		  if(pd->v[POLYMER_STRESS11])
		      ...
		      for(mode=0; mode<vn->modes; mode++)
			{
			  ...
			  if(fenep)
			    {
			      d_Pi->nn[p][q][j] += d_mup->nn[j]/lambda*(f_fenep*s[mode][p][q]-a_fenep*(double)delta(p,q));
			    }			  
			}		      
		    }
		}
      ...
      for(p=0; p<VIM; p++)
        {
		  ...
		  if(pd->v[POLYMER_STRESS11])
		      ...
		      for(mode=0; mode<vn->modes; mode++)
			{
			  ...
			  if(fenep)
			    {
			      d_Pi->F[p][q][j] += d_mup->F[j]/lambda*(f_fenep*s[mode][p][q]-a_fenep*(double)delta(p,q));
			    }			  
			}		      
		    }
		}
		      ...
		      if(pd->v[POLYMER_STRESS11])
			{
			  for(mode=0; mode<vn->modes; mode++)
			      ...
			      if(evss_f)
				{
			      ...
			      if(fenep)
				{
				  d_Pi->pf[p][q][a][j] += d_mup->pf[a][j]/lambda*(f_fenep*s[mode][p][q]-a_fenep*(double)delta(p,q));
				}			  
			    }		      
			}
		    }
		      ...
		      if(pd->v[POLYMER_STRESS11])
			{
			  for(mode=0; mode<vn->modes; mode++)
			      ...
			      if(evss_f)
				{
			      ...
			      if(fenep)
				{
				  d_Pi->v[p][q][b][j] += d_mup->v[b][j]/lambda*(f_fenep*s[mode][p][q]-a_fenep*(double)delta(p,q));
				}			  
			    }		      
			}
		    }
		      ...
		      if(pd->v[POLYMER_STRESS11])
			{
			  for(mode=0; mode<vn->modes; mode++)
			      ...
			      if(evss_f)
				{
			      ...
			      if(fenep)
				{
				  d_Pi->X[p][q][b][j] += d_mup->X[b][j]/lambda*(f_fenep*s[mode][p][q]-a_fenep*(double)delta(p,q));
				}			  
			    }		      
			}
		    }
		      ...
		      if(pd->v[POLYMER_STRESS11])
			{
			  for(mode=0; mode<vn->modes; mode++)
			      ...
			      if(evss_f)
				{
			      ...
			      if(fenep)
				{
				  d_Pi->C[p][q][w][j] += d_mup->C[w][j]/lambda*(f_fenep*s[mode][p][q]-a_fenep*(double)delta(p,q));
				}			  
			    }		      
			}
		    }
      ...
      for(p=0; p<VIM; p++)
        {
		  ...
		  if(pd->v[POLYMER_STRESS11])
		      ...
		      for(mode=0; mode<vn->modes; mode++)
			{
			  ...
			  if(fenep)
			    {
			      d_Pi->P[p][q][j] += d_mup->P[j]/lambda*(f_fenep*s[mode][p][q]-a_fenep*(double)delta(p,q));
			    }			  
			}		      
		    }
		}
