...
void
stress_no_v_dot_gradS_conf(double func[MAX_MODES][6],
                      double d_func[MAX_MODES][6][MAX_VARIABLE_TYPES + MAX_CONC][MDE],
                      const double dt,
                      const double tt
                     )
                          ...
                          if (lambda != 0.)
                            {
                             ...
                             if( lcwt != 0.) 
                               { 
                                 advection += lcwt*(s_dot_gt[a][b] + g_dot_s[a][b]);
                                 advection += (g[a][b] + gt[a][b]) * (ucwt - lcwt - 1.);
                               }
                             advection *= d_at_dT[j] * pd->etm[eqn][(LOG2_ADVECTION)];
                            }
                          ...
                          for ( q=0; q<VIM; q++)
                             {
  		                     ...
  		                     source1 = 0;
                                     if (alpha != 0.)
                                       {
                                        source1 = s[q][b] * (double)delta(a,p) + s[a][p] * (double)delta(b,q);
                                        source1 -= 2.0*(double)delta(a,p)*(double)delta(b,q);
                                        source1 *=  phi_j *  alpha / lambda; 
                                       }
                                     source += source1;
                                     source *= pd->etm[eqn][(LOG2_SOURCE)];
...
}/* END of routine stress_no_v_dot_gradS_conf                                */
/*****************************************************************************/
