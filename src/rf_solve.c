...
void
initial_guess_conf_to_log_conf(double *x, int num_total_nodes)
{
...
initial_guess_stress_to_log_conf(double *x, int num_total_nodes)
{
  ...
  for (int node = 0; node < num_total_nodes; node++) {
    ...
    idx = 0;
    for (int v = LOG_CONF11; v <= LOG_CONF22; v++) {
      log_idx[idx] = Index_Solution(node, v, 0, 0, -2);
      idx++;
    }
    ...
    x[log_idx[0]] = log_s[0][0];
    x[log_idx[1]] = log_s[0][1];
    x[log_idx[2]] = log_s[1][1];
