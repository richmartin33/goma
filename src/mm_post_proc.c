...
int nn_global;             /* Hopefully these post processing things get refactored into non-globals at some point */
...
//     shell_tangents_seeded(t0, t1, NULL, NULL);
  ...
  if ( LIFT_FORCE != -1 ) {
    ...
    if( dist_lift < (1.e-4 + 1.e-9))
      {
	lift_coeff =  3. * mu0 * 840. * 1.2 / (4. * M_PIE * dist_lift);
      }
    ...
    local_post[LIFT_FORCE + index] = lift_coeff2;
    local_lumped[LIFT_FORCE + index] = 1.;
    
    /*for (a = 0; a < dim; a++)
      {
	lift_force = lift_coeff * lift_dir[a];
	local_post[LIFT_FORCE + index] = lift_force;
	local_lumped[LIFT_FORCE + index] = 1.;
	index++;
	}*/
  }  
...
;	      set_nv_tkud(rd, index, 0, 0, -2, "TVP32","[1]",
  ...
  if (SHELL_NORMALS != -1  && ( Num_Var_In_Type[SHELL_ANGLE1] || Num_Var_In_Type[LUBP] ) )		
	...
	set_nv_tkud(rd, index, 0, 0, -2, "LIFT_FORCE0","[1]",
		    "Lift force x", FALSE);
	index++;
	index_post++;
      
	set_nv_tkud(rd, index, 0, 0, -2, "LIFT_FORCE1","[1]",
		    "Lift force y", FALSE);
	index++;
	index_post++;

	//set_nv_tkud(rd, index, 0, 0, -2, "LIFT_FORCE2","[1]",
	//	    "Lift force z", FALSE);
	//index++;
	//index_post++;
      }
